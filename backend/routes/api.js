...
router.post('/register', [
    ...
    try {
        ...
        await logAction('USER_REGISTER', email, 'SUCCESS', `Role: ${user.role}`, req);
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (user && (await bcrypt.compare(password, user.password))) {
            ...
            await logAction('LOGIN_ATTEMPT', email, 'FAILURE', 'Invalid credentials', req);
...
router.post('/products', protect, admin, [
    ...
    try {
        ...
        await logAction('DLL_INSERT_PRODUCT', req.user.email, 'SUCCESS', `Added: ${product.name}`, req);
...
router.put('/products/:id', protect, admin, async (req, res) => {
    try {
        ...
        await logAction('DLL_UPDATE_PRODUCT', req.user.email, 'SUCCESS', `Updated ID: ${req.params.id}`, req);
...
router.delete('/products/:id', protect, admin, async (req, res) => {
    try {
        ...
        await logAction('DLL_DELETE_PRODUCT', req.user.email, 'SUCCESS', `Deleted ID: ${req.params.id}`, req);
        res.json(result);
    } catch (err) {
        res.status(404).json({ message: err.message });
    }
...
router.post('/pay', protect, async (req, res) => {
    ...
    try {
        ...
        if (!cardNumber || cardNumber.length < 10 || !cvv) {
            await logAction('PAYMENT_GATEWAY', req.user.email, 'FAILURE', 'Invalid Card Details', req);
            return res.status(400).json({ message: 'Payment Declined: Invalid Card' });
        }

        // 2. Log the simulation event (Requirement 5.4)
        await logAction('PAYMENT_GATEWAY', req.user.email, 'SUCCESS', `Simulated charge of $${amount}`, req);

        // 3. Return success token (simulated)
        res.json({ message: 'Payment Approved', transactionId: 'TXN-' + Date.now() });
...
router.post('/checkout', protect, async (req, res) => {
    ...
    try {
        ...
        await logAction('ORDER_PLACED', req.user.email, 'SUCCESS', `Order ID: ${order._id} (${paymentMethod || 'Card'})`, req);

        res.status(201).json({ message: 'Order placed successfully', order });
    } catch (err) {
        console.error(err);
        await logAction('ORDER_ERROR', req.user ? req.user.email : 'unknown', 'FAILURE', err.message, req);
...
router.get('/analytics', protect, admin, async (req, res) => {
    try {
        ...
        const monthlySales = await Order.aggregate([
            { $group: { _id: { $month: "$timestamp" }, sales: { $sum: "$order_total" }, orders: { $sum: 1 } } },
            { $sort: { "_id": 1 } }
        ]);

        const topProducts = await Order.aggregate([
            { $unwind: "$items" },
            { $group: { _id: "$items.product_name", totalSold: { $sum: "$items.quantity" }, revenue: { $sum: { $multiply: ["$items.price", "$items.quantity"] } } } },
            { $sort: { revenue: -1 } },
            { $limit: 5 }
        ]);
        ...
        const categorySales = await Order.aggregate([
            { $unwind: "$items" },
            { $lookup: { from: "products", localField: "items.product_name", foreignField: "name", as: "productDetails" } },
            { $unwind: "$productDetails" },
            { $group: { _id: "$productDetails.category", value: { $sum: { $multiply: ["$items.price", "$items.quantity"] } } } }
        ]);

        res.json({ kpi: { totalRevenue, totalOrders, totalProducts }, monthlySales, topProducts, categorySales });
